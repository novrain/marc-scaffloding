...
function ratelimit(opts = {}) {
    ...
    return async function ratelimit(ctx, next) {
        const limitObj = opts.limit ? await opts.limit(ctx):{
            id:ctx.ip,
            max:10,
            duration:30000
        }
        const id = limitObj.id;
        const max = limitObj.max;
        const duration = limitObj.duration;

        //const id = opts.id ? await opts.id(ctx) : ctx.ip;
        //const max = opts.max ? await opts.max(ctx) : 10;
        if (undefined === id) return await next();
