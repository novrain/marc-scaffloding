...
Controller.prototype.error = function (ctx, err) {
...
Controller.prototype.send = function (ctx, context) {
    ctx.body = context.instance;
...
Controller.prototype._control = function (ctx) {
            ...
            hookChain = hookChain.then(function runHook(skip) {
                ...
                return functions.reduce(function (prev, current) {
                    return prev.then(function runHookFunction(skipNext) {
                        ...
                        return Promise.resolve(current.call(self, ctx, context))
    ...
    hookChain
        ...
        .catch(self.model.sequelize.ValidationError, function (err) {
            ...
            self.error(ctx, new errors.BadRequestError(err.message, errorList, err));
        })
        .catch(errors.EpilogueError, function (err) {
            self.error(ctx, err);
        })
        .catch(function (err) {
            self.error(ctx, new errors.EpilogueError(500, 'internal error', [err.message], err));
        });
    return hookChain;
};
