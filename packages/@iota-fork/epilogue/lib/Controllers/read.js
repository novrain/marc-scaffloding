...
Read.prototype.fetch = function(ctx, context) {
  ...
  Object.keys(criteria).forEach(function(attr) { delete ctx.params[attr]; });
  endpoint.attributes.forEach(function(attribute) {
    if (attribute in ctx.params) criteria[attribute] = ctx.params[attribute];
  });
