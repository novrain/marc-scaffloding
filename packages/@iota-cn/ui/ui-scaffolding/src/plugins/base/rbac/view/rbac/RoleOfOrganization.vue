...
export default {
    ...
    methods: {
        ...
        refetch() {
            ...
            if (organization) {
                this.$axios.silentGet(`/v1/api/authorizations/organizations/${organization}/roles?limit=${limit}&offset=${(page - 1) * limit}`, true)
                    .then((res) => {
        ...
        batchDelete(selectedRowKeys, selectedRows) {
            ...
            let url = `/v1/api/authorizations/organizations/${organization}/roles?ids=${ids.join(',')}`
        ...
        onAdd() {
            ...
            let url = `/v1/api/authorizations/organizations/${organization}/roles`
            return this.$axios.silentPost(url, { ids: newRoles.map(r => r.id) }, true)
                .then(() => {
                    ...
                    message.error("关联角色组织失败。")
                    return false
                })
        },

        findRoleNotBindToOrg(limit, offset) {
            let organization = this.organization
            let url = `/v1/api/authorizations/organizations/${organization}/not_assigned/roles?limit=${limit}&offset=${offset}`
    ...
    render() {
        const columns = [
            ...
            {
                title: '关联时间',
                dataIndex: 'organizationRole.createdAt',
        ...
        const addRelated = (
            <IiMultiAdd
                ref={'_addRelated'}
                fetchData={this.findRoleNotBindToOrg}
                nameField='name'
                columns={columnsAdd}
                // hidePagination={true}
                recordConvertor={row => {
