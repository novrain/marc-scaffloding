...
args.option('-h, --host [value]', 'server host')
    ...
    .option('-d, --domain-console [value]', 'iota domain')
    .option('-b, --domain-background [value]', 'iota background domain')
    .option('-g, --postgres [value]', 'postgres connection url')
    .option('-x, --postgres-background [value]', 'background postgres connection url')
    .option('-r, --redis-host [value]', 'redis server host')
    .option('-t, --redis-port [value]', 'redis server port')
    .option('-e, --elasticsearch-nodes [value]', 'elasticsearch nodes')
    .option('-u, --kubernets-server [value]', 'kubernets server')
    .option('-a, --auth-token [value]', 'kubernets service account token')
    .option('-n, --attachment-in-cloud', 'store attachment in cloud')
    .option('-l, --attachment-in-cloud-dev', 'store attachment in cloud dev')
    .option('-w, --orchestrator-api-proxy [value]', 'orchestartor api proxy')
